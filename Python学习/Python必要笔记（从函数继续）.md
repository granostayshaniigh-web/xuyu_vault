...
super().成员变量或方法()
```
20. 多态：多种状态，即完成某个行为（函数）时，使用不同的对象，得到不同的状态
![](assets/Python必要笔记（从函数继续）/file-20260818132658957.png)
------
![](assets/Python必要笔记（从函数继续）/file-20260818132811527.png)
21. 抽象类：由上述方法知道，父类中speak是==空实现==（pass)，具体的方法实现，由子类自行决定。父类只确定有哪些方法，子类来说明父类的方法的具体实现。
# 第三章
1. 异常处理：
```python
try:
