# 第一章
1. 计算机语言：
2. class：创建一个类，定义一个类。后面是类名。类的范围后面的大括号包含的
```java
//package:表示当前的类(HelloWorld)定义在那个包(practice.test)下面
package practice.test;  
//Class 是定义一个类,类是java项目中最基本的组成单元
public class HelloWorld {   
//public static void main,这是程序的主入口,会从主入口开始逐行往下执行
    public static void main(String[] args) {  
        System.out.println("Hello World");  
    }  
}
```
3. 程序中的数据，Java 中字面量分为：整数、小数、字符串、字符、布尔，空。
4. 变量是存储数据的，里面的数可以变。变量是指那个放数据的那个框
5. 经常发生改变的数据可以用变量来存储
6. 一个自己也就是1B等于八个比特位，int占四个字节也就是32个比特位
7. long类型的变量在定义的时候数字后面加一个L ，float后面要加一个F 。
8. 字符类型char定义的时候需要用单引号。单引号里面的字符只能是一个
9. 标识符只能是由数字，字母，下划线，以及符号，符号只能是$。
10. 小数参与计算可能不精确
```java
package practice.test;

import java.util.Scanner;

public class a_little_example {
    public static void main (String[] args){
        double a;
        double b;
        Scanner sc = new Scanner(System.in);
        System.out.println("Enter the first number a");
        a=sc.nextDouble();
        System.out.println("Enter the second number b");
        b=sc.nextDouble();
        System.out.println(a+b);
        System.out.println(a-b);
        System.out.println(a*b);
        System.out.println(a/b);
        System.out.println(a%b);
    }
}

D:\idea\bin\java.exe "-javaagent:D:\idea\IDEA_program\IntelliJ IDEA 2025.1\lib\idea_rt.jar=63304" -Dfile.encoding=UTF-8 -Dsun.stdout.encoding=UTF-8 -Dsun.stderr.encoding=UTF-8 -classpath D:\idea\IDEA_program\java_program\out\production\java_program practice.test.a_little_example
Enter the first number a
1.1
Enter the second number b
1.01
2.1100000000000003
0.09000000000000008
1.1110000000000002
1.0891089108910892
0.09000000000000008

进程已结束，退出代码为 0
```
11. 数字运算：类型不一样不能运算，需要转成同类型才能计算 ^ntmt1z
	- 隐式转换，小范围转成大范围，不需要写代码。例外：如果有byte、short类型的数据，计算时要转为int
	- 强制转换，大范围转成小范围，需要写代码，会导致精度丢失。int a；byte b=（byte）a；
12. 字符串拼接与数字拆分。
```java
package practice.test;  
import java.util.Scanner;  
public class num_divide {  
    public static void main (String[] args){  
        System.out.println("enter the number");  
        Scanner sc = new Scanner(System.in);  
        int seconds =sc.nextInt(); 
        //数字拆分 
        int hours=seconds/3600;  
        int minutes=seconds%3600/60;  
        int second=seconds%3600%60;
        //字符串拼接  
        System.out.println("小时:"+hours);  
        System.out.println("分钟:"+minutes);  
        System.out.println("秒:"+second);  
    }  
}
```
13. 逻辑运算符：逻辑与&、逻辑或|、逻辑非！、短路与&&、短路或||
14. 三元运算符
15. 运算符的优先级，小括号的优先级高于所有。
# 第二章
13. switch语句中小括号中的语句得到的是一个确定的值==（这个值可以是字符，整数不能是小数和长整数，枚举，字符串）==。这个确定值与case后面的值（==不能用变量表示，不能重复==）进行匹配，匹配上了就执行后面的语句。==并且default和case没有先后之分，也可以省略，所以没有和case匹配时，不会有输出。==case后面可以写==多个值==，用逗号隔开。->可以写在case后面,在大括号里面不用写break，但是冒号和->不能同时写。==yield程序最后的分号。==
# 第三章
13. 无限循环while（true）{}；for（；；）{}；do{}while（true）；其中true不能用1代替，java中int不能转化为bool
14. 生成随机数：
```java
Random r = new Random()
第一种写法：int n = r.nextInt();默认在int范围随机取值
第二种写法：int n = r.nextInt(n);默认在 0~n-1 范围随机取值
第三种写法：int n = r.nextInt(a,b);默认在 a~b-1 范围随机取值
```
# 第四章
15. 数组静态初始化：`int arr [] = new int [] {1,2,3};`
	数据类型  数组名 \[ ] = new 数据类型 \[ ] {数据值，数据值……}；
	简写：`int arr [] ={1,2,3}`;
16. 索引是每个小方格的编号，与里面存的值不一样。
17. `arr.length - 1`
18. 数组动态初始化`int arr [] = new int [3]`
# 第五章
19. 格式：定义在main方法的外面，类（public class 类名{ }）的里面。调用在main方法的里面。
```java
//定义：
public static 返回值类型 (传入形式参数1 ，传入形式参数2 ……){
	方法体；
	return 返回值；
}
//调用
方法名(传入实际参数1 ，传入实际参数2 ……);
```
20. 方法重载：在同一个类中，方法名相同，参数的类型、个数、顺序不同。不用看方法的返回值。在调用方法是如果没有参数对应，就会有参数的隐式转换。小范围转换为大范围。![Java必要笔记](java学习/Java必要笔记.md#^ntmt1z)
例外：在72个视频的11分钟
21. 方法之间不能嵌套
# 第六章
22. java程序运行过程：java文件 -> class字节码文件 ->结果
23. java运行在jvm虚拟机中的，使之可以跨平台运行，在任意平台运行同一个java文件。
24. java中**内存**分配：5个
- 栈内存：方法（函数）
![](assets/Java必要笔记/file-20260728204818471.png)
- ==堆内存==：new关键字（**每一块空间都拥有内存地址**）`int arr [] =new int []{1,2,3}==> int arr []={1,2,3}`
![](assets/Java必要笔记/file-20260728210026173.png)
- 方法区：字节码信息（class文件）
- 本地方法栈
- 程序计数器
22. 基本数据类型：记录的是真实的数据，传参时传递的也是真实数据。
23. 引用数据类型：记录的是内存地址，传参时传递的也是内存地址。
# 第七章
24. 对象：把相关的**零散**的数据和方法组织为一个**整体**来对待，用类（模版）来刻画的实例
25. 面向对象：利用对象进行软件开发
26. 类（javabean）：一个类来描述对象，其中属性是类中的变量（成员变量）
	- 同一类事物的属性必须是一致的
27. 创建对象格式：
```java
类名 对象名 = new 类名 ();
例：
Dog d1 = new Dog();
随后赋值：
d1.name = "旺财";  
d1.age = 3;  
d1.weight = 3.5;  
d1.color = "白色";
```
26. javabean:描述一类事物的类`public class Dog{}`
27. 带有main方法的类叫测试类`public static void main (String[] args){}`
28. javabean类可以写属性和行为；此处的行为就是方法（成员方法）（函数）,这个类可以做的事
29. 面向对象中数据安全的问题：private（私有化**所有**成员变量）、set（过滤错误数据）和get方法
![](assets/Java必要笔记/file-20260801204804615.png)

30. this关键字：区分成员变量和局部变量。就近原则
![](assets/Java必要笔记/file-20260801214556222.png)

31. 构造方法：创建对象时，**初始化**成员变量
- 格式：
```java
public Student(){  
}  
public Student(String name,int age,char gender,double hight){  
    this.name=name;  
    this.age=age;  
    this.gender=gender;  
    this.hight=hight;  
}
-----------------------------------------------------------------------------------
Student s2=new Student("张三",18,'男',1.8);
```
- 特点：
public后没有返回值类型，也没有返回值。其中方法名和类名相同，大小写也一致
- 执行时机：
由虚拟机自动调用，每次创建对象，就会调用一次构造方法
- 注意事项：
如果没有定义构造方法，系统将会给出一个默认的==无参数==构造方法
如果自己定义了，系统将不提供默认的构造方法。
无论是否调用，都要书写有参数和无参数构造方法（方法的重载）

ps：idea快捷键：alt+（fn）+insert

# 第八章
32. ![](assets/Java必要笔记/file-20260802214741684.png)
33. 把对象传递给方法实际传递的是对象的内存地址。
34. this 本质是指所在方法调用者的内存地址
![](assets/Java必要笔记/file-20260803210850283.png)
# 第九章
35. static修饰变量：static  -> 静态变量
```java
//两种方式访问静态变量：1.通过类名.变量名 2.通过对象名.变量名          
Student.teacherName="Ms.wang";  
s1.teacherName="Ms.wang";
```
36. static  -> 内存中的表示
	- 静态变量储存在堆内存中
	- 静态变量==随着类==的加载而加载的，优先于对象的出现。（ 静态变量属于类）
	- ![](assets/Java必要笔记/file-20260803215156537.png)
	- 第98个视频的第6分钟。
37. static修饰方法 -> 静态方法，该方法用于测试类和工具类中
	1. 工具类：不会描述事物，没有main方法，而是帮我们做一些事情的类
	2. `public class ArrayUtil{}`通常以Util结尾
	3. 私有化构造方法，让外界无法创建对象，因为它没有描述事物，创建对象是没有意义的
	4. 但是我们需要调用里面的方法，所以我们要把里面的方法设置为静态的，直接使用类名调用
38. 非静态方法只能通过对象名调用，所以先要创建对象。
39. static静态的注意事项：
	1. 静态方法只能访问静态变量和方法；
	2. 而非静态方法可以访问静态变量和方法以及非静态的成员变量和成员方法
	3. 静态方法中没有this关键字
	4. 即总结：非静态可以调用所有，静态只能调用静态
40. final 关键字：表示不可变，修饰变量、类、方法
	1. 变量字母要大写。
41. 枚举：
	1. 定义：是一个特殊的javabean类，这个类的对象是有限个
	2. 所以的枚举项都是默认使用public static final 来修饰的，因为有static，所以调用枚举可以用对象类名去调用
	3. 每一个枚举项,都是该枚举类的对象，以构造方法创建，即使没有在test类中调用，这个对象也已经创建
	4. 枚举类的==第一行==上必须是枚举项,枚举项之间用逗号隔开,以分号作为结尾
	5. 枚举类的构造方法必须是private修饰,不让外界创建本类的对象
	6. 编译器会给枚举类新增两个默认存在的方法:values()获取所有的枚举项,valueOf ()获取指定的枚举项
# 第十章
42. 面向对象的三大特征：封装（书写javabean时将属性和行为集中在一起）、继承、多态
43. 继承：
	1. 继承是类与类之间的一种==父子关系==，extens用于建立类与类之间的关系。既提高了代码的复用性，又可以在子类中添加其他功能，使子类更加强大。
```java
public class Student extends Person{}
Student是子类
Person 是父类
```
\
	2. 抽取相同的内容，向上画图，从上往下编程，先写父类
	3. 特点：一个子类只支持单继承，不支持多继承，但支持多层继承
	4. 子类直接继承的父类叫做直接父类，间接继承的爷爷叫做间接父类
	5. java中的顶级父类：Object类，每一个类都直接或间接继承于Object类
	6. ![](assets/Java必要笔记/file-20260815213531715.png)

44. 继承中成员变量或方法的查找顺序：就近原则。子类没有办法访问到间接父类里的变量或方法
![](assets/Java必要笔记/file-20260816212725261.png)
45. 方法重写：
	1. 注释与注解：前者是给人看的，后者@开头是给虚拟机看的，方法重写：注解@Override。