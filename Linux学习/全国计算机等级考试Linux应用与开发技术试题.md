...
__ B
- 在 Linux 和 Unix 系统中，将一个新的文件系统（如硬盘分区、U盘、光盘等）接入到现有的目录结构时，需要将其与一个已存在的目录关联起来，这个目录就被称为**挂载点（Mount Point）。**
- 虽然==挂载点通常是一个子目录==，但“子目录”是一个非常宽泛的概念，不能特指用于挂载文件系统的目录。
- 分区是==物理或逻辑磁盘==上的存储空间划分，属于存储设备层面，而不是“目录树中的位置”。
- 目录树是指整个文件系统的层级结构（从根目录 / 开始），而不是指某个具体的位置。
- 挂载的本质：在 Windows 中，新分区通常会被分配一个盘符（如 D:、E:）；而在 Linux 中，没有盘符的概念，==所有的文件系统都必须“挂载”到根目录 / 下的某个挂载点（目录）才能被访问。==
- ==注意事项：==用作挂载点的目录最好是空目录。如果该目录原本有文件，挂载新文件系统后，原有的文件会被隐藏，直到卸载（umount）后才会重新显示。
...
==第一个磁盘：/dev/hda==
==第二个磁盘：/dev/hdb==
备注：早期老式电脑常用的并行接口硬盘，现在已很少见。
. SCSI / SATA / USB 接口
命名前缀：sd
第一个磁盘：/dev/sda
第二个磁盘：/dev/sdb
备注：目前最主流的硬盘接口类型，涵盖了常见的机械硬盘、SATA固态硬盘以及U盘等。
. NVMe 接口
命名前缀：nvme
第一个磁盘：/dev/nvme0n1
第二个磁盘：/dev/nvme1n1
...
[netstat命令](Linux学习/Linux.md#8%202%20netstat命令)本机接口占用情况 D A
...
__ A
这是**全球最流行的开源 Web 服务器软件**，通常大家说"Apache"就是指它。
- **诞生时间**：1995 年
- **作用**：把你的网站内容（HTML、PHP、图片等）发布到网络上，让别人能通过浏览器访问。简单来说，它就是**连接用户浏览器和网站文件的"桥梁"**。
**27. Linux的日志文件通常保存在(  )**
<table>
...
<table>
  ...
  <tr><td>C. makefile文件中目标文件后跟的是源文件，最后是~~生成源文件~~的命令</td><td>D. make命令从makefile文件获取模块之间的依赖关系</td></tr>
</table>
__ C 目标文件
==**39. F==TP是什么协议？(  )**
<table>
...
__ C
- **A. 域名协议**：通常指的是 **DNS** (Domain Name System，域名系统)，它的作用是将人类易读的域名（如 `www.baidu.com`）解析为计算机能识别的 IP 地址。
- - **B. 动态主机配置**：指的是 **DHCP** (Dynamic Host Configuration Protocol，动态主机配置协议)，它的作用是自动给局域网内的设备分配 IP 地址、子网掩码、网关等网络参数。
- - **D. 文件解析**：这不是一个标准的网络协议名称。
**40. 下列哪一个不是Web应用开发框架？(  )**
<table>
 ...
 B
 **A. Spring**：是 Java 领域最核心、最流行的企业级应用开发框架。它包含了 Spring MVC、Spring Boot 等模块，是构建 Web 应用的绝对主力
 **B. Unity**：这是一个**跨平台的游戏引擎**（Game Engine），主要用于开发 2D/3D 电子游戏、VR/AR（虚拟现实/增强现实）应用以及交互式 3D 内容。它**不属于** Web 应用开发框架
 **C. Struts2**：是 Java EE 中经典的、基于 MVC 设计模式的 Web 应用框架，专门用于处理 Web 请求和构建 Web 应用（虽然现在逐渐被 Spring Boot 取代，但它依然是标准的 Web 框架）。
 **D. MyBatis**：是 Java 中非常优秀的**持久层框架**（主要用于数据库操作和
## 二、填空题（共 10 题）

41. ==按==照冯·诺依曼体系结构，传统的单核CPU由运算器和\_\_\_\_\_\_\_\_\_\_这两个主要部件组成。
	控制器
	
	 冯·诺依曼规定，一台完整的计算机必须由以下五个部分组成：
	 ==**运算器==（ALU）**：负责执行所有的算术运算（加减乘除）和逻辑运算（与或非）。
	 ==**控制器==（Control Unit）**：计算机的“指挥中心”，负责从内存中读取指令、解码，并指挥其他部件协同工作。（_注：现代计算机中，==运算器和控制器==被集成在一起，统称为 **CPU**_）。
	 ==**存储器==（Memory）**：即内存，用于存放**程序（指令）和数据**。
	 **输入设备（Input）**：将外部信息转化为计算机能识别的形式输入进来（如键盘、鼠标、麦克风、摄像头、网卡）。
	 **输出设备（Output）**：将计算机处理的结果展示给人类或发送给其他设备（如显示器、音响、打印机、网卡）。
	 **控制、运算、** 储存
	 
41. ==Linux==下的软件包可分为两种，分别是\_\_\_\_\_\_\_\_\_\_和二进制包。
	源码包
	**解析**：Linux 下的软件包主要分为两大类：**源码包**（如 `.tar.gz`，需要用户自己编译安装，灵活性强但耗时）和**二进制包**（如 RPM 包 `.rpm`、Debian 包 `.deb`，已经编译好，可以直接安装，方便快捷）。
	
42. 在shell中，\_\_\管道符_\_\_\_\_\_\_\_是由竖杠（|）代表，用来连接多条命令。
43. ==D==NS服务使用的端口是\_\_\_\_\_\_\_\_\_\_。
	DNS服务标准端口号为53
44. 使用\_\_\_sudo\_\_\_\_\_\_\_命令可以将用户身份临时改变为root。
45. 已知shell变量nu=aaa，现在希望nu的值是aaabbb，则应执行的命令nu=\_\_\_aaabbb\_\_\_\_\_\_\_。
	${nu}bbb
	==echo ${PATH}/abc==
	==输出：PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin:/root/bin/abc==
	作用主要是**明确界定变量名的边界**。使用 `${nu}` 可以明确告诉 Shell：“**这里的变量名到 `nu` 就结束了**，后面的 `bbb` 只是普通的字符串”。  因此，`${nu}bbb` 会被正确解析为：取出变量 `nu` 的值（`aaa`），并在其后追加字符串 `bbb`，最终拼接成 `aaabbb`。
	
46. 命令\_\_\_\_\_mount ==-a==\_\_\_\_\_用来装载==所有==在/etc/fstab中定义的文件系统。
47. 通过DHCP获得信息的机器没有固定IP地址，用\_\_\_\_\_\_\_\_\_\_命令增加网络接口时，系统动态获得一个IP地址，用\_\_\_\_\_\_\_\_\_\_命令减少网络接口时，系统就释放一个IP地址。
	**ifup；ifdown** | 【解析】`ifup` 启动网卡并获取IP，`ifdown` 停用网卡。
	
48. Vi打开文档后，想把10-15行中的第一个 `'abc'` 替换为 `'efg'`，应输入命令\_\_\_\_\_\_\_\_\_\_。
	：10,15s/abc/efg/
	**`:`** —— 进入末行模式（命令行模式）。
	 **`10,15`** —— 指定操作的**行范围**，表示从第 10 行到第 15 行。
	 **`s`** —— 替换命令（substitute 的缩写）。
	**`/abc/efg/`** —— 将查找到的 `abc` 替换为 `efg`。
	    - _注意：这里**没有**加 `g`（global）标志。在 Vim 中，如果不加 `g`，默认**只替换每一行中匹配到的第一个**字符串。这正好符合题目“把10-15行中的第一个 'abc' 替换”的要求。_
	题目中说“把10-15行中的第一个 'abc' 替换”，在 Vim 的考试和日常语境中，这通常指的是 **“对这 6 行中的每一行，都只替换该行内出现的第一个 abc”**。如果==加上 `g`== 标志
	（`:10,15s/abc/efg/g`），则会替换这 6 行中**所有**的 `abc`。
	 ^7f10zi
49. ==编译==器GCC会在系统默认的路径中（如/usr/lib）寻找所需要的库文件，当使用了\_\_\_\_\_\_\_\_\_选项指定库文件路径后，会首先到指定的目录去寻找相关库文件。
	L
	- 示例：`gcc main.c -L/home/user/mylibs -lmath`
    - 这告诉 GCC：“在链接时，除了去默认的 `/usr/lib` 或 `/lib` 找库之外，**优先**去 `/home/user/mylibs` 目录下寻找需要的库文件。”
...
```
用户当前在 `/` 目录下，进行如下操作：
(1) 在 `/tmp` 目录下创建一个名为 `target` 的目录，应输入命令：【51】mkdir target /tmp;
(2) `cd /tmp/target`
(3) 输入命令 `cp /tmp/source/test.c`，将 `test.c` 文件复制到当前目录;
==(4)== 调用命令【52】chmod +w test.c，将文件改为可写形式;

(5) `vi test.c` 启动编辑;
==(6)== 使用【53】\_\_\_\_\_\_\_\_\_\_命令把文件中的字符串 `"This"` 全都替换为 `"That"`;
[全国计算机等级考试Linux应用与开发技术试题](Linux学习/全国计算机等级考试Linux应用与开发技术试题.md#^7f10zi)
1,$s/This/That/g

(7) 将光标移到第4行，输入命令【54】dd_删除该行;
(8) 输入命令【55】:wq 保存并退出vi编辑器；
...
```bash
【62】__________#!/bin/bash
m=`echo $RANDOM`
n1=$【63】_m_________           $[ $m % 100 ]最前面的$,中间的两个空格
【64】while__________            while true
do
    【65】___read_______ "Please input a number: "n    read -p 
    if [ $n == $n1 ]     
    then
        【66】__break________
    【67】_if [ $n > $n1 ]  # (( )) 是 C 风格算术表达式，应该用 <=，而不是 -le。[ $n gt $n1 ]
    then
        echo "bigger"
        continue
    【68】_if [ $n < $n1 ]   else
    ...
    【69】__________             fi
【70】done__________
【71】_____echo_____ "You are right."
```
[Linux_fi](Linux学习/Linux.md#^z6bi7t)

echo 打印变量是取值，需要加上$
【65】read的-p
![Linux](Linux学习/Linux.md#^60rmau)
<br/>
...
C
==**38.== 用GCC编译一个名为“HelloWorld.c”的C程序文件，编译生成执行文件名为“Hello”，则相应的编译命令是(  )** 进行连接
<table>
...
41. 从资源管理的角度来看，操作系统具备五大基本功能：处理器管理、\_\_\_、\_\_\__、文件管理和作业管理。

	进==城设==文接

41. 在Linux操作系统中，所有被操作系统管理的资源，例如网络接口卡、磁盘驱动器、打印机、输入输出设备、普通文件或者目录都被看作是一个\_\_文件_。
42. Linux文件系统中，点(`.`)表示当前目录，点点(`..`)表示\_\_\上一级_\_目录。
43. ==安==装Linux系统对硬盘分区时，必须有两种分区类型：Linux文件系统分区（根分区）和\_\_\_\_\_\_\_\_分区。
	- 考试角度：Linux 安装必须的两个分区是 根分区 + swap 分区
...
46. ==用==一个命令行读取文件 `file` 第5~10行：\_\_\_\_\_\_\_\_\_\_。
```
...
（前10行）        （取最后6行）
```
47. ==列==出当前目录下以 `abc` **开头**的文件名，应使用命令\_\_ls -l |grep abc_\_\_\_\_\_\_\_。
```
 ls \| grep '^abc' 
 ls abc*          
```
48. ==系统==管理员以root账户登录，可以使用\_\_\_\_\__\_\_\_\_命令进入普通用户账号，而不必重新登录。
	exit 是退出当前 shell/会话的命令，不是切换用户的命令。
	su username	切换到指定普通用户（Switch User）
	su - username	切换用户并加载该用户的环境变量（推荐）
49. Linux中，可以用\_\_\_\_systemctl、service_\_\_\_\_\_命令来启动某个系统服务。
...
==(1)== 首先用户要用 scp 命令把本地文件目录 `/opt/myproject/` 上传至服务器的 `/opt/` 目录下，应输入：
   `[root@localhost]#` 【51】\_\_\_\_\_\_\_\_\_\_scp -r `/opt/myproject/` @10.1.1.10:`/opt/`

远程目标路径是 /opt/（系统目录），通常需要 root 权限才能写入，所以远程用户写 root。
scp -r /opt/myproject/ ==root==@10.1.1.10:/opt/

(2) 然后，用户使用 ssh 以root身份登录服务器，应输入：
   `[root@localhost]#` 【52】\_\_\_\_\_\_\_\_\_\_ssh root

ssh root==@==10.1.1.10

(3) 上传成功，进入服务器 `/opt/myproject/` 目录后，用户使用 gcc 编译该目录下的c语言源程序，使用【53】\_\_\_\_\_-g \_\_\_\_\_编译器选项产生调试信息，使用【54】\_\_\_\_\_\_\_\_\_\_选项==将指定目录下==的文件加入到程序库文件的搜索目录列表中。
...
```
(4) 加入调试选项进行编译后，启动gdb调试该程序，应输入：【55】\_\_\_gdb main.c\_\_\_\_\_\_\_

可执行文件后面没有.c
gbk main

(5) 在 `int k = add(i,j);` 处设置断点，应输入：【56】\_\_\_b 8\_\_\_\_\_\_\_
...
(7) 用户进一步将add函数从 `main.c` 文件中分离出来，放到单独的 `add.c` 文件中，并打算使用 `make` 命令进行程序的编译，需要编写【59】\_\_\_\_makefile_\_\_\_\_\_文件。
(8) 然后执行 `make` 命令进行编译，当用户需要清理编译生成的可执行程序和中间文件时，应执行【60】\_\_\_make clean_\_\_\_\_\_\_
...
【62】\_\_\_\_\_\_\_\_\_\_2:100
【63】\_\_\_\_\_\_\_\_\_\_3:90
【64】\_\_\_\_\_\_\_\_\_\_=====
【65】\_\_\_\_\_\_\_\_\_\_3:90
65  3:100
每次运行进程都不一样。所以题目一共三个进程
![Linux](Linux学习/Linux.md#^df60l4)
...
==ctime==（状态改变时间，不是创建时间）	ls -lc	文件元数据（权限、所有者、链接数等）最后改变的时间 ^c7grva
...
运行态		                正在 CPU 上执行
等待态（阻塞态）		等待某事件发生（如 I/O 完成、程序运行完成）
终止态		                进程执行完毕或被强制终止
**25. 存放用户账号基本信息的文件是(  )**
<table>
...
41. ==多==道程序环境下，操作系统分配资源以\_\_\_\_\_\_\_\_\_\_为基本单位。

进程

42. \_\_\_\_\_管道符\_\_\_\_\_是用来连接多条命令的，前一个命令的输出是下一个命令的输入。
43. 在Linux操作系统中，设备都是作为特殊的\_\_\_\_\文件_\_\_\_\_\_来访问。
44. ==M==VC框架将应用程序分成了三个核心部件：模型、\_\_\_\_\_\视图_\_\_\_\_、**控制器**。
45. ==命==令\_\_\_\_\_\_\_\_\_\_显示**文件系统**空间使用情况，命令\_\_\_\_\_df\_\_\_\_\_显示**目录或文件**占用磁盘空间量。

df du

46. 每块设备文件名由主设备号和从设备号描述。第三块IDE硬盘的设备名为\_\_\_\_\_hdc_\_\_\_\_，它上面的第二个主分区对应的文件名是\_\_\_\_hdc2_\_\_\_\_\_。
47. 将光盘 `/dev/cdrom` 卸载的命令是\_\_\_\_umount\_\_\_\_\_\_。

umount /dev/cdrom

48. 想知道当前目录的绝对路径，可以在字符界面下输入\_\_\_\_\_pwd_\_\_\_\_命令。
49. ==统==计当前有多少用户正在使用系统的一行命令是\_\_\_\_\_\_\_\_\_\_。

who | wc -l

50. 用GDB调试时，如果想把变量 `i=6` 在15行设置断点，应输入命令\_\_\_b 15 if i=6\_\_\_\_\_\_\_。
...
(1) 首先，用户应输入如下命令登录服务器：
`[root@10.1.1.60]#` 【51】\_\_ssh zhangsan@10.1.1.5_\_\_\_\_\_\_\_
==(2) 为==实现免密登录，可用【52】\_\_\_\_\_\_\_\_\_\_命令生成密钥对，在客户端保存私钥，服务器端保存公钥。

ssh-keygen

==(3) 登==录服务器后，用户启动emacs，先按 `<Ctrl>+x` 组合键，再按 `<Ctrl>`+【53】\_\_\_\_\_\_\_\_\_\_组合键，新建文件 `test.c`。

f
![Linux](Linux学习/Linux.md#^pb9q5l)

==(4) 编==辑文件后按 `<Ctrl>+x` 和 `<Ctrl>`+【54】\_\_\_s\_\_\_\_\_\_\_组合键保存并退出emacs。
(5) 接着用户编辑 `makefile` 文件如下：
```makefile
test: test.o
    gcc -o test test.o
test.o: test.c
    【55】__________ gcc -c test.c -o test.o 
clean:
    【56】__rm -r____ test test.o       
```
【56】#test 和 test.o 都是文件，不是目录，所以用 -f 而非 -r

(==6) 执==行make命令进行编译，编译成功后输入如下命令在后台运行test程序：
`[zhangsan@10.1.1.5 ~]$` 【57】\_\_\_\_make\_\_\_\_\_\_

test是可执行文件，所以只需要./test就可以运行，在后台中则需要加上&
./test &

==(7) 用==户使用【58】\_\_\_\_\_\_ps\_\_\_\_命令查看当前登录产生的进程（如下所示）：
```
...
==(8) 由==于 `test` 程序执行时间过长，用户输入以下命令==强制结束该进程==：
`[zhangsan@10.1.1.5 ~]$` 【59】\_\_\_\_\_\_\_\_\_\_

kill 1677

(9) 之后执行【60】\_\_\_\_make clean\_\_\_\_\_\_清理编译生成的可执行程序和中间文件。

**2. 编写shell脚本实现石头、剪刀、布游戏如下:**
==随机数==
```bash
【61】__________#!bin/bash                         #!/bin/bash
#编写脚本,实现人机<石头,剪刀,布>游戏
#出拳的可能性保存在一个数组中，game[0],game[1],game[2]分别是3中不同的可能
game=(石头 剪刀 布)
#通过随机数获取计算机的出拳
num=【62】__________                      $[ ROADOM % 3 ] 
#通过num和game确定计算机的出拳
computer=【63】_game[num]_________        ${game[$num]}/${game[num]}
echo "请根据下列提示选择您的出拳手势"
echo "1.石头"
echo "2.剪刀"
echo "3.布"
【64】____read__ "请选择1-3:" person       read -p
case $person 【65】__________                  in
1)
    if [ $num -eq 0 ]
    then
        echo "平局"
...
2)
    if [ $num -eq 0 ]
    then
        【66】__________echo "计算机赢"
    elif [ $num -eq 1 ]
    then
        【67】__________echo "平局"
    else
        【68】__________echo "你赢"
    fi;;
3)
    if [ $num -eq 0 ]
    then
...
C B
```
正则表达式:ab*c  →  匹配 "ac", "abc", "abbc", "abbbc", ...(0个或多个b)
通配符：ab*c  →  匹配 "ac", "adc", "assc", "a123c", ...
关键点：正则表达式* 必须跟在某个字符后面，修饰的是它左边的那个字符。
🔹 通配符：管文件名，* 是万能王，简单粗暴
...
41. 在Linux的两种链接文件方式中，\_\_\_\_硬链接\_\_\_\_\_\_不能实现对目录链接。
42. ==在==Linux操作系统中，设备文件分为块设备文件和\_\_\_\_\_\_\_\_\_\_设备文件。
![Linux](Linux学习/Linux.md#^25wagn)

43. vi编辑器的默认模式是\_\_\_\_命令模式\_\_\_\_\_\_。任何时候，不管用户处于何种工作模式，按下ESC键，即可进入该模式。
44. ==执==行一个shell命令时，通常会自动打开三个标准文件，标准输入文件(stdin)，它对应的设备是\_\_\_\_\_\_\_\_\_\_，另外两个则分别是标准输出文件(stdout)和标准错误输出文件(stderr)。

|        标准文件        | 文件描述符 |    对应设备     | 说明          |
| :----------------: | :---: | :---------: | :---------- |
|  **stdin**（标准输入）   |   0   |   **键盘**    | 程序从这里读取输入数据 |
|  **stdout**（标准输出）  |   1   | **显示器（屏幕）** | 正常输出结果      |
| **stderr**（标准错误输出） |   2   | **显示器（屏幕）** | 错误信息输出      |

| 重定向符号  |      含义       | 示例                   |
| :----: | :-----------: | :------------------- |
|  `<`   | 输入重定向（从文件读入）  | `sort < file.txt`    |
|  `>`   | 输出重定向（覆盖写入文件） | `ls > output.txt`    |
|  `>>`  | 输出重定向（追加写入文件） | `ls >> output.txt`   |
|  `2>`  |    错误输出重定向    | `cmd 2> error.log`   |
| `2>&1` |  错误输出合并到标准输出  | `cmd > all.log 2>&1` |
键盘

41. ==某==文件的权限为：`drw-r--r--`，用数值形式表示该权限，则该八进制数为\_\_\_\_\_\_\_\_\_\_。

就是说的是数字表达

644
41. `mv` 命令用来将文件从一个目录移到另一个目录中，还可以对文件\_\_重命名_\_\_\_\_\_\_\_。
42. 在linux系统中，所有的变量都默认是字符串，如果进行算术运算，将变量a和b的值相加，赋给变量c，其语法格式为\_\_\_\_c=$\[ a + b ]\_\_\_\_\_\_。
==也可以是：let c=a+b==

43. ==给==出一分命令统计当前工作目录下所有以**m开头的C语言源文件**的数量\_find . -name "" | wc -l\_\_\_\_\_\_\_\_\_。
ls m*.c | wc -l
44. 在linux系统中，用gcc编译c语言的源文件 `f1.c`，生成可执行文件 `f1` 的命令是\_\_\_\_gcc f1.c -o f1\_\_\_\_\_\_。
45. ==Lin==ux系统的守护进程及多数日志文件是由\_\_\_\_\_\_\_\_\_\_服务来统一管理的，只要各个进程将信息给予这个服务，它就会自动地把日志按特定的格式记录到不同的日志文件中。
...
【51】\_\_\_\_\_\_\_\_\_\_bgi bin
`[student@localhost~]$ ls s[m-ox-z]*`
【52】\_\_\_\_\_\_\_\_\_\_source sys
`[student@localhost~]$ ls ?i?`
【53】\_\_\_\_\_\_\_\_\_\_bin lib
`[student@localhost~]$ ls [!a-go-z]*`
【54】\_\_\_\_\_\_\_\_\_\_include iomega lib
`[student@localhost~]$ ls [eo]???`
【55】\_\_\_\_\_\_\_\_\_\_exam objs
...
【56】__________
do
    if 【57】__________(($x>$max)) 
    then
        max=$x
    fi
    if 【58】__________(($x<$max)) 
    then
        min=$x
    fi
done
...
则运行该脚本，应执行以下命令：
【59】\_\_\_\_\_\_\_\_\_\_chmod a+x sh01/chmod u+x sh01
【60】\_\_\_\_\_\_\_\_\_\_./sh01

![Linux](Linux学习/Linux.md#^zy3cil)

```
实际运行过程
[student@localhost~]$ ./sh01
please input the data:5      ← 第一次 read（循环外）
                      8      ← while read x（第1次循环）
                      3      ← while read x（第2次循环）
                      12     ← while read x（第3次循环）
^D                           ← 按 Ctrl+D，read 失败，循环结束
max=12
min=3
```
