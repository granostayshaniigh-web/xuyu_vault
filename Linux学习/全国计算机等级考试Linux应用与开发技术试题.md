...
（前10行）        （取最后6行）
```
41. 列出当前目录下以 `abc` 开头的文件名，应使用命令\_\_ls -l |grep abc_\_\_\_\_\_\_\_。
```
 ls \| grep '^abc' 
 ls abc*          
```
...
(7) 用户进一步将add函数从 `main.c` 文件中分离出来，放到单独的 `add.c` 文件中，并打算使用 `make` 命令进行程序的编译，需要编写【59】\_\_\_\_\makefile_\_\_\_\_\_文件。
