...
（前10行）        （取最后6行）
```
41. ==列==出当前目录下以 `abc` **开头**的文件名，应使用命令\_\_ls -l |grep abc_\_\_\_\_\_\_\_。
```
 ls \| grep '^abc' 
 ls abc*          
```
41. ==系统==管理员以root账户登录，可以使用\_\_\_\_\_exit\_\_\_\_\_命令进入普通用户账号，而不必重新登录。
	exit 是退出当前 shell/会话的命令，不是切换用户的命令。
	su username	切换到指定普通用户（Switch User）
	su - username	切换用户并加载该用户的环境变量（推荐）
42. Linux中，可以用\_\_\_\_\systemctl_\_\_\_\_\_命令来启动某个系统服务。
...
game=(石头 剪刀 布)
#通过随机数获取计算机的出拳
num=【62】__________                      $[ ROADOM % 3 ] RANDOM
#通过num和game确定计算机的出拳
